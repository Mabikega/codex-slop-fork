...
impl ChatWidget {
    ...
    fn pop_latest_queued_user_message(&mut self) -> Option<UserMessage> {
        if let Some(queued_message) = self.queued_user_messages.pop_back() {
            ...
            Some(queued_message.into_user_message())
        } else {
    ...
    pub(crate) fn maybe_send_next_queued_input(&mut self) {
        ...
        while !self.is_user_turn_pending_or_running() {
            ...
            match queued_message.action {
                QueuedInputAction::Plain => {
                    self.submit_user_message_with_metadata(
    ...
    fn open_status_line_setup(&mut self) {
        ...
        let saved_account_limit_formatter = crate::slop_fork::SavedAccountStatusLineFormatter::load(
