...
#[cfg(test)]
use codex_protocol::protocol::TurnAbortReason as ProtocolTurnAbortReason;
...
impl ChatWidget {
    ...
    fn on_task_complete_for_turn(
        ...
        let was_pilot_turn = turn_id.is_some_and(|turn_id| {
            self.slop_fork_ui
        ...
        let pilot_effects = turn_id
            ...
            .map(|turn_id| {
                self.slop_fork_ui.on_pilot_turn_completed(
                    &slop_fork_context,
                    turn_id,
    ...
    pub(crate) fn on_interrupted_turn(&mut self, reason: TurnAbortReason) {
        self.finish_interrupted_turn(reason, /*show_default_interrupt_message*/ true);
    }
    ...
    pub(crate) fn restore_thread_input_state(&mut self, input_state: Option<ThreadInputState>) {
        ...
        if let Some(input_state) = input_state {
            ...
            self.queued_user_message_metadata = input_state.queued_user_message_metadata;
    ...
    fn add_boxed_history(&mut self, cell: Box<dyn HistoryCell>) {
        ...
        if !keep_placeholder_header_active
            && !cell.display_lines(MAX_RATATUI_PARAGRAPH_WIDTH).is_empty()
        {
    ...
    fn queue_user_message_with_metadata(
        ...
        self.queue_user_message_with_metadata_and_options(
            user_message,
            metadata,
    ...
    fn queue_user_message_with_metadata_and_options(
        &mut self,
        user_message: UserMessage,
        metadata: QueuedUserMessageMetadata,
        action: QueuedInputAction,
    ...
    fn slop_fork_context(&self) -> SlopForkUiContext {
        SlopForkUiContext {
            ...
            sandbox_policy: self.config.legacy_sandbox_policy(),
            file_system_sandbox_policy: self.config.permissions.file_system_sandbox_policy(),
        ...
        for effect in effects {
            match effect {
                ...
                SlopForkUiEffect::SubmitPilotTurn {
                    ...
                    if self.submit_op(AppCommand::slop_fork_pilot_turn(prompt)) {
                ...
                SlopForkUiEffect::SubmitAutoresearchTurn {
                    ...
                    if self.submit_op(AppCommand::slop_fork_autoresearch_turn(prompt)) {
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        let _accepted = self
            .submit_user_message_with_metadata(user_message, QueuedUserMessageMetadata::default());
    }
    ...
    fn submit_user_message_with_metadata(
        ...
        self.submit_user_message_with_metadata_history_and_shell_escape_policy(
            user_message,
            metadata,
            UserMessageHistoryRecord::UserMessageText,
    ...
    fn submit_user_message_with_metadata_history_and_shell_escape_policy(
        &mut self,
        user_message: UserMessage,
        metadata: QueuedUserMessageMetadata,
        history_record: UserMessageHistoryRecord,
    ...
    fn dispatch_event_msg(
        ...
        if let EventMsg::TurnAborted(ev) = msg {
            match ev.reason {
                ProtocolTurnAbortReason::Interrupted => {
                    let reason = if ev
                        .turn_id
                        .as_deref()
                    ...
                    let runtime_reason = match reason {
                        TurnAbortReason::Interrupted => ProtocolTurnAbortReason::Interrupted,
                        TurnAbortReason::BudgetLimited => ProtocolTurnAbortReason::BudgetLimited,
                    };
                    self.apply_slop_fork_runtime_event(runtime_event_from_turn_abort_reason(
                        ev.turn_id.as_deref(),
                        runtime_reason,
                ...
                ProtocolTurnAbortReason::Replaced => {
                    self.submit_pending_steers_after_interrupt = false;
                    self.pending_steers.clear();
                    self.refresh_pending_input_preview();
                ...
                ProtocolTurnAbortReason::ReviewEnded => {
                    self.finish_interrupted_turn(
                        TurnAbortReason::Interrupted,
                ...
                ProtocolTurnAbortReason::BudgetLimited => {
    ...
    pub(crate) fn maybe_send_next_queued_input(&mut self) -> bool {
        ...
        while !self.is_user_turn_pending_or_running() {
            let use_default_metadata = !self.rejected_steers_queue.is_empty();
            let Some((queued_message, history_record)) = self.pop_next_queued_user_message() else {
                break;
            };
            let metadata = if use_default_metadata {
                QueuedUserMessageMetadata::default()
            } else {
            ...
            match queued_message.action {
                QueuedInputAction::Plain => {
                    submitted_follow_up = self
                        .submit_user_message_with_metadata_history_and_shell_escape_policy(
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.codex_op_target {
            CodexOpTarget::Direct(codex_op_tx) => {
                ...
                if let Err(e) = codex_op_tx.send(op.clone()) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            CodexOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::CodexOp(op.clone()));
            }
        }
...
impl Renderable for ChatWidget {
    ...
    fn cursor_style(&self, area: Rect) -> crossterm::cursor::SetCursorStyle {
        self.bottom_pane
            .cursor_style(self.layout_for_area(area).bottom_pane_inner)
    }
}
