...
pub(crate) trait HistoryCell: std::fmt::Debug + Send + Sync + Any {
    ...
    fn prepare_viewport(&self, width: u16) -> PreparedHistoryCellViewport {
        PreparedHistoryCellViewport::from_lines(
    ...
    fn desired_height_for_mode(&self, width: u16, mode: HistoryRenderMode) -> u16 {
        PreparedHistoryCellViewport::from_lines(self.display_lines_for_mode(width, mode), width)
            .desired_height()
    }
...
mod tests {
    ...
    fn oversized_history_line_does_not_panic_during_measure_or_render() {
        ...
        let height =
            std::panic::catch_unwind(AssertUnwindSafe(|| cell.desired_height(/*width*/ 80)));
        assert!(
            height.is_ok(),
