...
fn empty_saved_account_status_line_formatter() -> crate::slop_fork::SavedAccountStatusLineFormatter
{
    crate::slop_fork::SavedAccountStatusLineFormatter::load(
        std::path::Path::new("/non-existent"),
        &[],
    )
}
...
async fn token_usage_update_uses_runtime_context_window() {
    ...
    assert_eq!(
        chat.status_line_value_for_item(
            ...
            &empty_saved_account_status_line_formatter(),
        ),
...
async fn status_line_git_summary_items_render_values() {
    ...
    assert_eq!(
        chat.status_line_value_for_item(
            crate::bottom_pane::StatusLineItem::PullRequestNumber,
            &empty_saved_account_status_line_formatter(),
        ),
    ...
    assert_eq!(
        chat.status_line_value_for_item(
            crate::bottom_pane::StatusLineItem::BranchChanges,
            &empty_saved_account_status_line_formatter(),
        ),
...
async fn raw_output_status_line_value_only_shows_when_enabled() {
    ...
    assert_eq!(
        chat.status_line_value_for_item(
            crate::bottom_pane::StatusLineItem::RawOutput,
            &empty_saved_account_status_line_formatter(),
        ),
        None
    );
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let init = ChatWidgetInit {
        ...
        auth_manager: codex_core::test_support::auth_manager_from_auth_with_home(
