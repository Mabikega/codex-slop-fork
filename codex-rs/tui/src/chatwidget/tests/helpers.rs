...
pub(super) async fn make_chatwidget_manual(
    ...
    let test_cwd = cfg.cwd.to_path_buf();
    let mut widget = ChatWidget {
        ...
        layout_pass_revision: std::cell::Cell::new(0),
        ...
        status_line_project_root_name_cache: Some(CachedProjectRootName {
            cwd: test_cwd,
            root_name: None,
        }),
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.auth_manager = codex_core::test_support::auth_manager_from_auth_with_home(
        codex_core::CodexAuth::create_dummy_chatgpt_auth_for_testing(),
        chat.config.codex_home.clone().to_path_buf(),
    );
