...
impl ChatWidget {
    ...
    pub(super) fn handle_slash_command_with_args_dispatch(
        ...
        if matches!(cmd, InlineCommand::Slash(_)) {
            self.bottom_pane.record_pending_slash_command_history();
        }
    }
    ...
    pub(super) fn dispatch_command_with_args(
        ...
        if trimmed.is_empty()
            && let InlineCommand::Slash(cmd) = cmd
        {
            self.dispatch_command(cmd);
            return;
        }
        ...
        match cmd {
            ...
            InlineCommand::Auto | InlineCommand::Autoresearch | InlineCommand::Pilot => {
                self.dispatch_prepared_command_with_args(
                    cmd,
                    PreparedSlashCommandArgs {
                        args,
    ...
    fn dispatch_prepared_command_with_args(
        ...
        let trimmed = args.trim();
        match cmd {
            InlineCommand::Slash(SlashCommand::Fast) => match trimmed.to_ascii_lowercase().as_str()
            {
            ...
            InlineCommand::Slash(SlashCommand::Ide) => {
                self.handle_ide_command_args(trimmed);
            }
            InlineCommand::Slash(SlashCommand::Keymap) => {
                match trimmed.to_ascii_lowercase().as_str() {
                    "" => self.open_keymap_picker(),
                    "debug" => {
            ...
            InlineCommand::Slash(SlashCommand::Raw) => {
                match trimmed.to_ascii_lowercase().as_str() {
                    "on" => {
            ...
            InlineCommand::Slash(SlashCommand::Goal) if !trimmed.is_empty() => {
                if !self.config.features.enabled(Feature::Goals) {
                    return;
                }
            ...
            InlineCommand::Slash(SlashCommand::Side) if !trimmed.is_empty() => {
                let Some(parent_thread_id) = self.thread_id else {
                    self.add_error_message(
        ...
        if source == SlashCommandDispatchSource::Live
            && cmd != InlineCommand::Slash(SlashCommand::Goal)
        {
            self.bottom_pane.drain_pending_submission_state();
        }
    }
