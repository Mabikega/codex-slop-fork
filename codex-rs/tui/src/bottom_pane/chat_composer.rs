...
mod tests {
    ...
    fn set_connector_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_text_content("$no".to_string(), Vec::new(), Vec::new());
        assert!(matches!(composer.active_popup, ActivePopup::None));
        ...
        assert!(
            popup.selected_mention().is_some(),
            "expected mention popup to have a selection"
        );
        let mention = composer
            .mention_items()
            .into_iter()
            .find(|mention| mention.path.as_deref() == Some("app://connector_1"))
            .expect("expected connector mention to be present after refresh");
    ...
    fn set_skill_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_text_content("$co".to_string(), Vec::new(), Vec::new());
        assert!(matches!(composer.active_popup, ActivePopup::None));
        ...
        let mentions: Vec<_> = composer
            ...
            .filter(|mention| {
                mention
                    .path
                    .as_ref()
                    .is_some_and(|path| !path.starts_with("slop-fork://command/"))
            })
            .collect();
        ...
        snapshot_composer_state_with_width(
            ...
            |composer| {
                composer.set_skill_mentions(Some(vec![SkillMetadata {
                    ...
                    dependencies: None,
                    policy: None,
                    path_to_skills_md: AbsolutePathBuf::try_from(PathBuf::from(
                        "/tmp/repo/auto/SKILL.md",
                    ))
    ...
    fn inline_slash_command_can_be_recalled_after_recording_pending_history() {
        ...
        match result {
            InputResult::CommandWithArgs(cmd, args, text_elements) => {
                assert_eq!(cmd, InlineCommand::Slash(SlashCommand::Plan));
