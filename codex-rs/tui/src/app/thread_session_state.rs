...
impl App {
    ...
    pub(super) async fn session_state_for_thread_read(
        ...
        let local_permission_profile = self.current_permission_profile();
        let permission_profile = response
            .permission_profile
            .clone()
            .map(Into::into)
            .unwrap_or_else(|| local_permission_profile.clone());
        let active_permission_profile = if permission_profile == local_permission_profile {
