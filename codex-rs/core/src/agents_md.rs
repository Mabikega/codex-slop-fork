...
impl<'a> AgentsMdManager<'a> {
    ...
    async fn read_agents_md(&self, fs: &dyn ExecutorFileSystem) -> io::Result<Option<String>> {
        let agents_md_overlay = load_project_doc_overlay(self.config)?;
        let max_total = self.config.project_doc_max_bytes;

        if max_total == 0 {
            let mut inline_sections = agents_md_overlay.prefix_sections;
            inline_sections.extend(agents_md_overlay.suffix_sections);
        ...
        let mut parts = agents_md_overlay.prefix_sections;
        ...
        parts.extend(agents_md_overlay.suffix_sections);
    ...
    async fn agents_md_paths(
        ...
        extend_project_doc_paths(
            ...
            &search_dirs,
            &candidate_filenames,
            &agents_md_overlay.additional_filenames,
            &mut found,
        )
        .await?;
