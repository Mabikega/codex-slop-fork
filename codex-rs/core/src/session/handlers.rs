...
pub(super) async fn user_input_or_turn_inner(
        ...
        .await
    {
        Ok(_) => {
            ...
            if let Some((turn_context, _)) = sess.active_turn_context_and_cancellation_token().await
            {
...
async fn slop_fork_assistant_turn(
    ...
    sess.maybe_emit_unknown_model_warning_for_turn(turn_context.as_ref())
        .await;
    sess.refresh_mcp_servers_if_requested(&turn_context, None)
        .await;
        ...
        let should_exit = async {
            match sub.op.clone() {
                ...
                Op::SlopForkPilotTurn { prompt } => {
                    slop_fork_pilot_turn(&sess, sub.id.clone(), prompt).await;
                    false
                }
                Op::SlopForkAutoresearchTurn { prompt } => {
                    slop_fork_autoresearch_turn(&sess, sub.id.clone(), prompt).await;
                    false
                }
