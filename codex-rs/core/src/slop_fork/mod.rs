...
pub(crate) fn remove_saved_account(
    ...
    if removed && was_active {
        let _ = auth::logout(codex_home, AuthCredentialsStoreMode::Ephemeral)?;
        if auth_credentials_store_mode != AuthCredentialsStoreMode::Ephemeral {
            let _ = auth::logout(codex_home, auth_credentials_store_mode)?;
        }
    }
    Ok(removed)
}
...
pub fn record_rate_limit_snapshot_for_auth(
    ...
    record_rate_limit_snapshot_for_auth_with_raw(codex_home, auth, snapshot, None);
}
