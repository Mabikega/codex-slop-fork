...
impl AutomationRegistry {
    ...
    pub fn record_delivery(
        ...
        self.initialize_runtime_for_spec(scope, &spec, now);
        self.apply_successful_action(runtime_id_to_update, &spec, now, None)?;
        self.save_runtime_states()?;
        Ok(true)
    }
...
impl CronExpression {
    fn parse(input: &str) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            minutes: CronField::parse(fields[0], 0, 59, "minute", false)?,
            hours: CronField::parse(fields[1], 0, 23, "hour", false)?,
            days_of_month: CronField::parse(fields[2], 1, 31, "day-of-month", false)?,
            months: CronField::parse(fields[3], 1, 12, "month", false)?,
            days_of_week: CronField::parse(fields[4], 0, 7, "day-of-week", true)?,
        })
    }
