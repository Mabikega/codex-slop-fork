...
impl AutoresearchDiscoveryReason {
    pub fn parse(value: &str) -> Option<Self> {
        match value {
            ...
            "portfolio_refresh" => Some(Self::PortfolioRefresh),
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::PortfolioRefresh => "portfolio_refresh",
    ...
    pub fn label(self) -> &'static str {
        match self {
            ...
            Self::PortfolioRefresh => "portfolio refresh",
...
pub fn build_discovery_prompt(
    ...
    let approach_tracking_line = if state.mode.is_open_ended() {
        "- When you identify a concrete candidate worth tracking, call `autoresearch_log_approach` to register or update it.\n"
    } else {
        "- In optimize mode, record concrete candidate ideas in the discovery summary or ideas file instead of calling open-ended-mode approach tools.\n"
    };
    format!(
        "Autoresearch directive: run one bounded discovery pass separate from the benchmark loop.\n\
         ...
         - After `autoresearch_log_discovery` succeeds, do not re-sync the repo, do not call more tools, and do not continue auditing. End the turn immediately with the concise synthesis.\n\
        ...
        approach_tracking_line = approach_tracking_line,
