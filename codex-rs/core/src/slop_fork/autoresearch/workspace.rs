...
impl AutoresearchWorkspace {
    ...
    fn restore_discard_git(&self) -> Result<String, String> {
        ...
        let mut tracked_paths = git_changed_paths(&self.workdir, /*staged*/ true)?;
        tracked_paths.extend(git_changed_paths(&self.workdir, /*staged*/ false)?);
...
fn git_dirty_paths_excluding_protected(workdir: &Path) -> Result<Vec<String>, String> {
    let mut paths = git_changed_paths(workdir, /*staged*/ true)?;
    paths.extend(git_changed_paths(workdir, /*staged*/ false)?);
...
pub(crate) fn refresh_snapshot(workdir: &Path, snapshot_root: &Path) -> Result<(), String> {
    match fs::remove_dir_all(snapshot_root) {
        Ok(()) => {}
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {}
...
pub(crate) fn restore_snapshot(workdir: &Path, snapshot_root: &Path) -> Result<(), String> {
