...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let automation_manager = SlopForkAutomationManager;
        let autoresearch_manager = SlopForkAutoresearchManager::default();
        let pilot_manager = SlopForkPilotManager::default();
        let thread_processor = ThreadRequestProcessor::new(
            ...
            state_db.clone(),
            automation_manager.clone(),
            autoresearch_manager.clone(),
            pilot_manager.clone(),
        );
    ...
    async fn handle_initialized_client_request(
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            ClientRequest::AutomationList { params, .. } => {
                self.thread_processor
                    .automation_list(request_id.clone(), params)
                    .await
            }
            ClientRequest::AutomationUpsert { params, .. } => {
                self.thread_processor
                    .automation_upsert(request_id.clone(), params)
                    .await
            }
            ClientRequest::AutomationDelete { params, .. } => {
                self.thread_processor
                    .automation_delete(request_id.clone(), params)
                    .await
            }
            ClientRequest::AutomationSetEnabled { params, .. } => {
                self.thread_processor
                    .automation_set_enabled(request_id.clone(), params)
                    .await
            }
            ClientRequest::PilotRead { params, .. } => {
                self.thread_processor
                    .pilot_read(request_id.clone(), params)
                    .await
            }
            ClientRequest::PilotStart { params, .. } => {
                self.thread_processor
                    .pilot_start(request_id.clone(), params)
                    .await
            }
            ClientRequest::PilotControl { params, .. } => {
                self.thread_processor
                    .pilot_control(request_id.clone(), params)
                    .await
            }
            ClientRequest::AutoresearchRead { params, .. } => {
                self.thread_processor
                    .autoresearch_read(request_id.clone(), params)
                    .await
            }
            ClientRequest::AutoresearchStart { params, .. } => {
                self.thread_processor
                    .autoresearch_start(request_id.clone(), params)
                    .await
            }
            ClientRequest::AutoresearchControl { params, .. } => {
                self.thread_processor
                    .autoresearch_control(request_id.clone(), params)
                    .await
            }
            ClientRequest::SlopForkAssistantTurnStart { params, .. } => {
                self.thread_processor
                    .slop_fork_assistant_turn_start(request_id.clone(), params)
                    .await
            }
            ...
            ClientRequest::SavedAccountActivate { params, .. } => {
                self.account_processor.saved_account_activate(params).await
            }
            ClientRequest::SavedAccountRemove { params, .. } => {
                self.account_processor.saved_account_remove(params).await
            }
