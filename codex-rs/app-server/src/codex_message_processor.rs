...
impl CodexMessageProcessor {
    ...
    async fn resolve_pilot_thread_id(
        &self,
        thread_id: &str,
        allow_archived: bool,
        ...
        match find_thread_path_by_id_str(&self.config.codex_home, &thread_id_str).await {
            ...
            Err(err) => {
                return Err(JSONRPCErrorError {
                    ...
                    message: format!("failed to resolve pilot thread {thread_id}: {err}"),
                    data: None,
                });
            }
        }
    ...
    async fn resolve_active_pilot_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<CodexThread>), JSONRPCErrorError> {
        let thread_id = ThreadId::from_string(thread_id).map_err(|err| JSONRPCErrorError {
    ...
    async fn pilot_read(&self, request_id: ConnectionRequestId, params: PilotReadParams) {
        let thread_id = match self.resolve_pilot_thread_id(&params.thread_id, true).await {
    ...
    async fn pilot_start(&self, request_id: ConnectionRequestId, params: PilotStartParams) {
        let (thread_id, conversation) =
            match self.resolve_active_pilot_thread(&params.thread_id).await {
                Ok(thread) => thread,
                Err(error) => {
                    self.outgoing.send_error(request_id, error).await;
                    return;
