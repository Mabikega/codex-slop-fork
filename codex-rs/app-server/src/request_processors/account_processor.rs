...
impl AccountRequestProcessor {
    ...
    pub(crate) async fn saved_account_activate(
        &self,
        params: SavedAccountActivateParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.saved_account_activate_response(params)
            .await
            .map(|response| Some(response.into()))
    }

    pub(crate) async fn saved_account_remove(
        &self,
        params: SavedAccountRemoveParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.saved_account_remove_response(params)
            .await
            .map(|response| Some(response.into()))
    }
    ...
    async fn saved_account_activate_response(
        &self,
        params: SavedAccountActivateParams,
    ) -> Result<SavedAccountActivateResponse, JSONRPCErrorError> {
        let activated = self
            .auth_manager
            .activate_saved_account(&params.account_id)
            .map_err(|err| internal_error(format!("failed to activate saved account: {err}")))?;
        if activated {
            Self::maybe_refresh_remote_installed_plugins_cache_for_current_config(
                &self.config_manager,
                &self.thread_manager,
        ...
        Ok(SavedAccountActivateResponse { activated })
    }

    async fn saved_account_remove_response(
        &self,
        params: SavedAccountRemoveParams,
    ) -> Result<SavedAccountRemoveResponse, JSONRPCErrorError> {
        let removed = self
            .auth_manager
            .remove_saved_account(&params.account_id)
            .map_err(|err| internal_error(format!("failed to remove saved account: {err}")))?;
        if removed {
            Self::maybe_refresh_remote_installed_plugins_cache_for_current_config(
                &self.config_manager,
                &self.thread_manager,
        ...
        Ok(SavedAccountRemoveResponse { removed })
    }
