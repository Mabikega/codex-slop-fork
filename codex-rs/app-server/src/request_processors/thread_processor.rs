...
fn slop_fork_pilot_error(
    prefix: &'static str,
) -> impl FnOnce(SlopForkPilotError) -> JSONRPCErrorError {
    move |err| match err {
        SlopForkPilotError::InvalidRequest(message) => invalid_request(message),
        SlopForkPilotError::Io(err) => internal_error(format!("{prefix}: {err}")),
    }
}

fn slop_fork_autoresearch_error(
    prefix: &'static str,
) -> impl FnOnce(SlopForkAutoresearchError) -> JSONRPCErrorError {
    move |err| match err {
        SlopForkAutoresearchError::InvalidRequest(message) => invalid_request(message),
        SlopForkAutoresearchError::Io(err) => internal_error(format!("{prefix}: {err}")),
    }
}
...
pub(crate) struct ThreadRequestProcessor {
    ...
    pub(super) automation_manager: SlopForkAutomationManager,
    pub(super) autoresearch_manager: SlopForkAutoresearchManager,
    pub(super) pilot_manager: SlopForkPilotManager,
}
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn automation_list(
        ...
        let data = self
            ...
            .await
            .map_err(|err| internal_error(format!("failed to list automations: {err}")))?;
        self.outgoing
    ...
    pub(crate) async fn automation_upsert(
        &self,
        request_id: ConnectionRequestId,
        ...
        let automation = self
            ...
            .await
            .map_err(|err| internal_error(format!("failed to upsert automation: {err}")))?;
    ...
    pub(crate) async fn automation_delete(
        ...
        let deleted = self
            ...
            .await
            .map_err(|err| internal_error(format!("failed to delete automation: {err}")))?;
        self.outgoing
    ...
    pub(crate) async fn automation_set_enabled(
        ...
        let automation = self
            .automation_manager
            .set_enabled(&self.config.codex_home, &cwd, &thread_id, params)
            .await
            .map_err(|err| internal_error(format!("failed to update automation: {err}")))?;
    ...
    pub(crate) async fn pilot_read(
        &self,
        request_id: ConnectionRequestId,
        params: PilotReadParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        let (thread_id, conversation) = self
        ...
        let read = self
            ...
            .await
            .map_err(|err| internal_error(format!("failed to read pilot state: {err:?}")))?;
        self.outgoing
            .send_response(
                request_id,
                PilotReadResponse {
    ...
    pub(crate) async fn pilot_start(
        &self,
        request_id: ConnectionRequestId,
        ...
        let run = self
            ...
            .await
            .map_err(slop_fork_pilot_error("failed to start pilot run"))?;
        self.outgoing
        ...
        self.send_pilot_notification(
            ...
            PilotUpdatedNotification {
                thread_id: thread_id.to_string(),
                update_type: PilotUpdateType::Started,
                run: Some(run),
    ...
    pub(crate) async fn pilot_control(
        ...
        let (updated, run, update_type, message) = self
            .pilot_manager
            .control(
                &self.config.codex_home,
                &thread_id,
            ...
            .await
            .map_err(slop_fork_pilot_error("failed to update pilot run"))?;
        self.outgoing
            .send_response(
                request_id,
    ...
    pub(crate) async fn autoresearch_start(
        &self,
        request_id: ConnectionRequestId,
        ...
        let (cwd, has_active_turn) = if let Some(conversation) = conversation {
            ...
            let cwd = self.resolve_unloaded_fork_thread_cwd(thread_id).await?.ok_or_else(|| {
        ...
        let run = self
            ...
            .await
            .map_err(slop_fork_autoresearch_error("failed to start autoresearch"))?;
        self.outgoing
            .send_response(
                request_id,
        ...
        self.send_autoresearch_notification(
            ...
            AutoresearchUpdatedNotification {
                thread_id: thread_id.to_string(),
                update_type: AutoresearchUpdateType::Started,
                run: Some(run),
    ...
    pub(crate) async fn autoresearch_control(
        &self,
        request_id: ConnectionRequestId,
        ...
        let (fallback_workdir, has_active_turn) = if let Some(conversation) = conversation {
            ...
            (
                self.resolve_unloaded_fork_thread_cwd(thread_id).await?,
                false,
            )
        };
        let (updated, run, update_type, message) = self
            ...
            .await
            .map_err(slop_fork_autoresearch_error(
                "failed to control autoresearch",
            ))?;
        self.outgoing
            .send_response(
                request_id,
                AutoresearchControlResponse {
                    updated,
                    run: run.clone(),
                },
            )
    ...
    pub(crate) async fn autoresearch_read(
        &self,
        request_id: ConnectionRequestId,
        ...
        let read = self
            ...
            .read(&self.config.codex_home, &thread_id, has_active_turn)
            .await
            .map_err(|err| internal_error(format!("failed to read autoresearch state: {err}")))?;
        self.outgoing
            .send_response(
                request_id,
    ...
    pub(crate) async fn slop_fork_assistant_turn_start(
        ...
        let thread_id = ThreadId::from_string(&params.thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        self.reject_pending_unload(thread_id, "slopFork/assistantTurnStart")
            .await?;
        ...
        let turn_id = self
            .submit_core_op(&request_id, thread.as_ref(), op)
            .await
            .map_err(|err| internal_error(format!("failed to start fork assistant turn: {err}")))?;
        self.outgoing
    ...
    async fn thread_has_active_turn(&self, thread_id: &ThreadId, thread: &CodexThread) -> bool {
        let has_active_turn = {
            let thread_state = self.thread_state_manager.thread_state(*thread_id).await;
            let thread_state = thread_state.lock().await;
            thread_state.active_turn_snapshot().is_some()
        };
        has_active_turn || matches!(thread.agent_status().await, AgentStatus::Running)
    }
    ...
    async fn resolve_fork_thread_id(
        ...
        if !allow_archived {
            return Err(invalid_request(format!(
                "thread is not active: {thread_id}"
            )));
        }
        match self
            .thread_store
            ...
            .await
        {
            ...
            Err(err) => Err(internal_error(format!(
                "failed to resolve thread {thread_id}: {err}"
            ))),
        }
    }
    ...
    async fn resolve_unloaded_fork_thread_cwd(
        ...
        match self
            ...
            .await
        {
            Ok(thread) => Ok(Some(thread.cwd)),
            Err(ThreadStoreError::ThreadNotFound { .. })
            | Err(ThreadStoreError::InvalidRequest { .. }) => Ok(None),
            Err(err) => Err(internal_error(format!(
                "failed to resolve thread {thread_id}: {err}"
            ))),
        }
