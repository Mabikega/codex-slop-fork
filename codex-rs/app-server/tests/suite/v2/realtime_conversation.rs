...
fn assert_call_create_multipart(
    ...
    let multipart_body = body
        .strip_prefix(sdp_prefix)
        .context("multipart body missing SDP preamble")?;
    let (actual_offer_sdp, session_body) = multipart_body
        .split_once(session_separator)
        .context("multipart body missing session part")?;
    let actual_session = session_body
        .strip_suffix(body_suffix)
        .context("multipart body missing closing boundary")?;

    assert_eq!(actual_offer_sdp, offer_sdp);
    let expected_session: Value =
        serde_json::from_str(session).context("expected session JSON should parse")?;
    let actual_session: Value =
        serde_json::from_str(actual_session).context("actual session JSON should parse")?;
