...
fn thread_start_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()> {
    std::thread::Builder::new()
        .name("remote-thread-store-regression".to_string())
        ...
        .spawn(|| {
            let runtime = tokio::runtime::Builder::new_current_thread()
                ...
                .build()?;
            runtime.block_on(
                thread_start_with_non_local_thread_store_does_not_create_local_persistence_inner(),
            )
        })?
        .join()
        .map_err(|_| anyhow::anyhow!("remote thread-store regression thread panicked"))?
}

async fn thread_start_with_non_local_thread_store_does_not_create_local_persistence_inner()
-> Result<()> {
...
fn assert_no_local_persistence_artifacts(codex_home: &Path) -> Result<()> {
    ...
    // Fork-owned managers keep their state under codex_home as documented fork
    // storage. These files are not thread persistence artifacts.
    entries.remove(".codex-slop-fork-automation-state.json");
    entries.remove(".codex-slop-fork-automation-state.lock");
    entries.remove(".codex-slop-fork-autoresearch-state.json");
    entries.remove(".codex-slop-fork-autoresearch-state.lock");
    entries.remove(".codex-slop-fork-pilot-state.json");
    entries.remove(".codex-slop-fork-pilot-state.lock");
    entries.remove("config-slop-fork.toml");
    assert_eq!(
        entries,
        BTreeSet::from([
